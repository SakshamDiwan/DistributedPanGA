...
static void dump_tuple(int contig_id, int64 position, int strand,
                       const uint8 *kmer_bytes, int kbytes)
{
    pthread_mutex_lock(&DUMP_MUTEX);
    fprintf(DUMP_FP, "%d %lld %d ", contig_id, (long long) position, strand);
    for (int b = 0; b < kbytes; b++)
        fprintf(DUMP_FP, "%02x", kmer_bytes[b]);
    fprintf(DUMP_FP, "\n");
...
//  In setup_thread_plain, neq[i] (after the in-place transform) packs bases [i, i+3]
//  with the base at position i in the high 2 bits. The stored record's byte b corresponds
//  to neq[bost+4b], so byte b packs bases [j+4b, j+4b+3]. Total span: bases [j, j+39].
//  Returns 0 on success, -1 if the k-mer would extend past the contig.
static int pack_forward_kmer(GDB *gdb, int contig_id, int64 j, uint8 *out)
{
    int64  begb = j;
    int64  endb = j + KMER;        //  exclusive
    ...
    bases = (uint8 *) Get_Contig_Piece(gdb, contig_id, (int) begb, (int) endb, NUMERIC, buf);
    if (bases == NULL)
        return -1;
    ...
    {
        int p = i * 4;
        out[i] = (uint8) ((bases[p] << 6) | (bases[p+1] << 4)
                        | (bases[p+2] << 2) |  bases[p+3]);
    }
    return 0;
}

//  Pack the reverse-complement 40-mer at per-contig position j into 10 bytes.
//  In setup_thread_plain's RC branch, after `bost += TMER`, n = neq + (j+8). The byte
//  written for position b is Comp[n[-4b]] = Comp[neq[j+8-4b]], i.e. the reverse
//  complement of the forward 4-mer covering bases [j+8-4b, j+11-4b].
//  Total span on the forward strand: bases [j-28, j+11] (40 bases).
static int pack_rc_kmer(GDB *gdb, int contig_id, int64 j, uint8 *out)
{
    int64  begb = j - 28;
    int64  endb = j + 12;          //  exclusive
    int    clen = gdb->contigs[contig_id].clen;
    char   buf[64];
    uint8 *bases;
    int    b;
    ...
    //  bases[k] = forward NUMERIC base at contig position (j-28 + k), for k in [0, 40).
    //  For byte b in [0, 10), source positions are [j+8-4b, j+11-4b],
    //  which become local indices [36-4b, 39-4b].
    for (b = 0; b < KBYTES; b++)
    {
        int p3 = 36 - 4*b;
        uint8 fwd4 = (uint8) ((bases[p3]   << 6) | (bases[p3+1] << 4)
                            | (bases[p3+2] << 2) |  bases[p3+3]);
        out[b] = (uint8) Comp[fwd4];
    }
    return 0;
}
	      ...
	      j = i-SOFF;
              ...
              if (j <= lnK)
                { w = (w<<2|p>>6);
                  ...
                  { uint8 kmer_buf[KBYTES];
                    if (pack_forward_kmer(gdb, r, j, kmer_buf) == 0)
                      dump_tuple(r, j, 0, kmer_buf, KBYTES);
                  }
                }
              if (j >= KMT)
                { c = (c<<2|(3-(p&0x3)));
                  ...
                  { uint8 kmer_buf[KBYTES];
                    /* setup_thread_plain stores position (j+TMER) for RC records,
                       and the RC k-mer covers forward bases [j-28, j+11].
                       To match GIXshow output we emit position (j+TMER). */
                    if (pack_rc_kmer(gdb, r, j, kmer_buf) == 0)
                      dump_tuple(r, j + TMER, 1, kmer_buf, KBYTES);
                  }
                }
#ifdef DEBUG_MAP
              printf("\n");
...
{ 
  printf("\n=== Distribution Phase ===\n");
  printf("Scanning %d contigs across %d threads\n", gdb->ncontig, NTHREADS);
  printf("Distributing to %d partitions\n", NPARTS);
  fflush(stdout);
  ...
  { int64 t, *buck;
    ...
    printf("\n=== Bucket Distribution ===\n");
    printf("Total k-mers sampled: %lld\n", Buckets[0][NUM_BUCK-1]);
    printf("K-mer split points:\n");
    fflush(stdout);
    for (int z = 0; z <= NPARTS; z++)
      printf("  Part %d: buckets [%d, %d)\n", z, Ksplit[z], (z < NPARTS) ? Ksplit[z+1] : NUM_BUCK);
...
{ uint8 *sarray;
  ...
  { int   i, j;
    ...
    // Print per-bucket counts from scan_thread for verification
    printf("\nScanBuckets\n");
    for (i = 0; i < NUM_BUCK; i++)
      { int64 total = 0;
      ...
      { int64 prev, next;
        ...
        for (p = kbeg; p < kend; p++)
          { int64 b_start = prev;
            ...
            if (p == 870)
              { FILE *df = fopen("gixmake_bucket870.txt","w");
                if (df != NULL)
                  { fprintf(df,"# bucket 870: %lld records, swide=%d\n",
                            next - b_start, swide);
                    for (int64 r = b_start; r < next; r++)
                      { uint8 *rec = sarray + r*swide;
                        for (int q = 0; q < swide; q++)
...
{ 
  ...
  printf("\n=== Contigs ===\n");
  for (int z = 0; z < gdb->ncontig && z < 20; z++)
    printf("contig[%d]: len=%d boff=%lld\n", z, gdb->contigs[z].clen, gdb->contigs[z].boff);
  if (gdb->ncontig > 20)
    printf("... (%d more contigs)\n", gdb->ncontig - 20);
  fflush(stdout);
  ...
  { int64 npost, range, cum, t;   //  Compute DB split into NTHREADS parts
    ...
    printf("\n=== Work Distribution ===\n");
    printf("Total sequence: %lld bp\n", gdb->seqtot);
    printf("Splitting into %d threads:\n", NTHREADS);
    for (int z = 0; z <= NTHREADS; z++)
      printf("  Thread %d: contigs [%d, %d), starting at post %lld\n", z, DBsplit[z], (z < NTHREADS) ? DBsplit[z+1] : gdb->ncontig, DBpost[z]);
    fflush(stdout);
  ...
  { int64 nels;
    ...
    printf("\n=== Sort Configuration ===\n");
    printf("PostBytes = %d (bytes per position)\n", PostBytes);
    printf("ContBytes = %d (bytes per contig+sign)\n", ContBytes);
    printf("NPARTS = %d (sort partitions)\n", NPARTS);
    printf("KBYTES = %d (bytes per packed k-mer)\n", KBYTES);
    printf("MBYTES = %d (bytes per masked k-mer)\n", MBYTES);
    fflush(stdout);
  ...
  {
    const char *dump_path = getenv("GIXMAKE_DUMP_PATH");
    if (dump_path == NULL) dump_path = "gix_dump.tuples";
  ...
  if (DUMP_FP != NULL) fclose(DUMP_FP);
